...
- Predictive evidence is weak after accounting for overlapping-return dependence. For example, the 13-week COT-index regression has an OLS t-statistic of **−2.87**, but a HAC/Newey-West t-statistic of **−1.22**.
...
- **COT index:** a full 156-week minimum history for the trailing three-year range.
...
- **Inference:** OLS t-statistics plus HAC/Newey-West robustness statistics for overlapping forward returns.
- **Validation:** explicit checks for duplicate keys, invalid roll anchors, stale matches, non-monotone dates, and out-of-range signal values.
...
- The notebook is the authoritative executable analysis.
- The PDF report summarizes the final post-run numerical results and interpretation.
- `futspread/` contains the supporting spread-analysis code used for Question 2.
- Local caches, WRDS credentials, downloaded raw data, and Python bytecode should not be committed.
